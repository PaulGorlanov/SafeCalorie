...
3. Не предлагай дополнительные блюда к уже заполненным приёмам.`);let{minMenuCalories:y,targetMenuCalories:b}=H(c,d);return g+=`
...
Ответь текстом на русском. Можно markdown. Не JSON.`,n}function Be(e,t){if(e?.profile?.error)return`Профиль не заполнен. Попроси заполнить профиль в настройках.`;let{remaining:n,goals:r}=G(e),i=r.calories>0?n.calories/r.calories:0,a=`Ты диетолог-помощник в приложении SafeCalorie.
...
Не JSON.`,a}function Ve(e,t){if(e?.profile?.error)return`Профиль не заполнен. Попроси заполнить профиль в настройках.`;let{remaining:n}=G(e),r=Math.min(Math.max(n.calories,0),300),i=`Ты диетолог-помощник в приложении SafeCalorie.
...
{"text":"краткий ответ","items":[{"name":"...","calories":250,"protein":3,"carbs":56,"fat":0,"portionWeight":200,"mealType":"dinner"}]}`,_t=`
Если пользователь пишет "добавь", "добавь эти блюда", "они не добавлены", "те что выше" — это ссылка на последние блюда из истории.
Если в истории есть калории/БЖУ по блюдам, восстанови блюда из этой истории и верни непустой items.`;function vt({chatContext:e,profile:t,diaryToday:n}){let r=S(),i=t??r.profile,a=g(),o=h(a,-1),s=`Ты помощник по учёту калорий. Помогай пользователю следить за питанием.`;return i&&(s=`Ты помощник по учёту калорий. Профиль: вес ${i.weight} кг, рост ${i.height} см, норма ${i.dailyCalories} ккал/день.`),s+=I(e),s+=`
Сегодня: ${a}, вчера: ${o}.`,n?.consumed&&(s+=`
Уже съедено сегодня: ${Math.round(n.consumed.calories)} ккал (Б ${Math.round(n.consumed.protein)} / У ${Math.round(n.consumed.carbs)} / Ж ${Math.round(n.consumed.fat)} г).`),s+=gt,s}var yt=`
Пользователь прислал фото еды. Оцени калории и БЖУ по схеме nutrition_proposal.`;function bt(e){let t=e.replace(/\s+/g,` `).trim();return t?t.length>40?`${t.slice(0,40)}…`:t:`Новый чат`}async function xt({providerId:e,baseURL:t,token:n,messages:r,model:i}){let a=D(e)?[{jsonMode:!0},{jsonMode:!1}]:[{responseFormat:P},{responseFormat:fe},{jsonMode:!0}],o=null;for(let s of a)try{return await E({providerId:e,baseURL:t,token:n,messages:r,model:i,...s})}catch(e){o=e}throw o??Error(`Не удалось получить ответ`)}async function St({providerId:e,baseURL:t,token:n,messages:r,model:i}){let a=r.filter(e=>e.role!==`system`),o=e=>typeof e==`string`?e:Array.isArray(e)?e.filter(e=>e?.type===`text`&&typeof e.text==`string`).map(e=>e.text).join(` `):``,s=e=>/(\d+)\s*(?:ккал|kcal)/i.test(e)&&/(?:белк|углевод|жир|бжу)/i.test(e),c=a.slice(-10).map(e=>({role:e.role,content:o(e.content).trim()})).filter(e=>e.content&&s(e.content)).slice(-3).map(e=>`${e.role}: ${e.content.slice(0,500)}`).join(`
`),l;try{l=await ie({providerId:e,baseURL:t,token:n,messages:r,model:i,tools:dt(),executeTool:X,maxRounds:4})}catch{l=await xt({providerId:e,baseURL:t,token:n,messages:r,model:i})}if(q(Y(l.content).proposals).length)return l;let u=[{role:`system`,content:Z},...a];try{l=await E({providerId:e,baseURL:t,token:n,messages:u,model:i,jsonMode:!0})}catch{try{l=await E({providerId:e,baseURL:t,token:n,messages:u,model:i,jsonMode:!1})}catch{return l}}if(q(Y(l.content).proposals).length||!c)return l;let d=[{role:`system`,content:`${Z}
${_t}
Опирайся на этот контекст:
${c}
Верни JSON по схеме meal_proposals с непустым items.`},...a];try{l=await E({providerId:e,baseURL:t,token:n,messages:d,model:i,responseFormat:P})}catch{try{l=await E({providerId:e,baseURL:t,token:n,messages:d,model:i,jsonMode:!0})}catch{}}return l}function Ct({response:e,wantsLogFood:t,hasImage:n}){if(e.usedTools&&!t)return{displayContent:e.content.trim(),proposals:[]};if(t||n){let t=Y(e.content);return{displayContent:t.text||e.content.trim(),proposals:q(t.proposals)}}let r=N(e.content);if(r)return{displayContent:r.text,proposals:q(r.proposals)};let i=ut(e.content),a=i.proposals?.length?q(i.proposals):i.proposal?q([{...i.proposal,status:`pending`}]):[];return{displayContent:i.text,proposals:a}}function Q(e){if(!e)return null;let t={name:String(e.name??``),...k({calories:e.calories,protein:e.protein,carbs:e.carbs,fat:e.fat,portionWeight:e.portionWeight??e.weight})};return e.suggestedMealType&&(t.suggestedMealType=e.suggestedMealType),e.status&&(t.status=e.status),t}function $(e){return(e??[]).map(Q).filter(Boolean)}function wt(e){let t={...e};return t.nutritionProposals?(t.nutritionProposals=$(t.nutritionProposals),t.nutritionProposal=t.nutritionProposals[0]??void 0):t.nutritionProposal&&=Q(t.nutritionProposal),t}var Tt=t(`chat`,()=>{let e=a([]),t=a(null),n=a([]),r=a(!1),i=a(null),o=a(null);function s(){t.value&&localStorage.setItem(T,String(t.value))}async function c(){e.value=await m.chats.orderBy(`updatedAt`).reverse().toArray()}async function l(e=t.value){if(!e){n.value=[];return}n.value=await m.chatMessages.where(`chatId`).equals(e).sortBy(`createdAt`)}async function u(e){return await m.chatMessages.where(`chatId`).equals(e).count()===0}async function d(){await c();let t=e.value[0];return t&&await u(t.id)?(await _(t.id),t.id):p()}async function f(){if(await c(),!e.value.length){await p(`Чат 1`);return}let n=Number(localStorage.getItem(T)),r=e.value.some(e=>e.id===n)?n:e.value[0].id;t.value=r,s(),await l(r)}async function p(e=`Новый чат`){let t=Date.now(),n=await m.chats.add({title:e,createdAt:t,updatedAt:t});return await c(),await _(n),n}async function _(e){t.value=e,s(),i.value=null,await l(e)}async function v(r){if(await m.chatMessages.where(`chatId`).equals(r).delete(),await m.chats.delete(r),await c(),t.value===r){if(e.value.length){await _(e.value[0].id);return}t.value=null,n.value=[],localStorage.removeItem(T),await p(`Чат 1`)}}async function y(e,t={}){let n=Date.now();await m.chats.update(e,{updatedAt:n,...t}),await c()}async function b(){await f()}function x({hasImage:e=!1,chatContext:t=null}={}){let n=S().profile,r=g(),i=h(r,-1),a=`Ты помощник по учёту калорий. Помогай пользователю следить за питанием.`;return n&&(a=`Ты помощник по учёту калорий. Профиль пользователя: вес ${n.weight} кг, рост ${n.height} см, возраст ${n.age}, пол ${n.gender===`male`?`мужской`:`женский`}. Дневная норма калорий: ${n.dailyCalories} ккал.`),e?a+=yt:(a+=I(t),a+=`
...
Для вопросов о дневнике — обычным текстом, без JSON.`),a}function C(){let e=w(),t=se(),n=t.activeProvider,r=ne(n),i=re(n,e.getToken(n));if(!i&&ae(n))throw Error(`Укажите API-ключ для ${r.name} в настройках`);if(n===`lmstudio`&&!t.activeModel)throw Error(`Выберите модель LM Studio в настройках`);return{providerId:n,token:i,baseURL:ce(n,{lmstudioBaseURL:t.lmstudioBaseURL}),provider:r}}function ee(e,t){if(!t)return e||` `;let n=[];return e?n.push({type:`text`,text:e}):n.push({type:`text`,text:`Оцени калории и состав блюда на фото. Ответь кратко на русском.`}),n.push({type:`image_url`,image_url:{url:t}}),n}function D(){return he(n.value.map(e=>e.imageData?{role:e.role,content:ee(e.content,e.imageData)}:{role:e.role,content:e.content}))}async function O(e){return(await m.chats.get(e))?.context??null}async function k(t,n){await m.chats.update(t,{context:n});let r=e.value.find(e=>e.id===t);r&&(r.context=n)}function ue(e){for(let t=e-1;t>=0;t--){let e=n.value[t];if(e.role===`user`)return e.imageData??null}return null}async function A(e,t){let r=wt(t);await m.chatMessages.update(e,r);let i=n.value.findIndex(t=>t.id===e);i!==-1&&(n.value[i]={...n.value[i],...r})}function j(e,t){return e?{usageCostRub:e.costRub??0,usageTotalTokens:e.totalTokens??0,usageKind:t}:{}}async function M({providerId:e,model:t,kind:n,usage:r}){r&&await le().logUsage({providerId:e,model:t,kind:n,usage:r})}async function N({text:e=``,imageData:a=null}={}){let s=e.trim();if(!s&&!a)return;t.value||await p();let c=t.value,l,u,d;try{({providerId:l,token:u,baseURL:d}=C())}catch(e){throw i.value=e.message,e}let f=se(),h=lt({imageData:a}),_=h?null:ge(s),v=!_&&!h&&(ct(s,{messages:n.value})||rt(s)),b=s||(a?`Оцени калории на фото`:``);n.value.length===0&&s&&await y(c,{title:bt(s)});let S=o.value;o.value=null;let w={chatId:c,role:`user`,content:b,imageData:a??void 0,providerId:l,createdAt:Date.now(),...j(S,`transcribe`)};w.id=await m.chatMessages.add(w),n.value.push(w),await y(c),r.value=!0,i.value=null;try{let e=await O(c),t=n.value.slice(0,-1),r;r=_?Pe(_,await Ne(_.id),e,s):v?[{role:`system`,content:vt({chatContext:e,diaryToday:await X(`get_diary_summary`,{date:g()})})},...D()]:[{role:`system`,content:x({hasImage:h,chatContext:e})},...D()];let i;if(h)try{i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,responseFormat:fe})}catch{try{i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,jsonMode:!0})}catch{i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,jsonMode:!1})}}else if(_)i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,jsonMode:!1});else if(v)i=await St({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel});else if(oe(l,s,{hasImage:h,messages:t}))try{i=await ie({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,tools:dt(),executeTool:X})}catch(e){if(i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,jsonMode:!1}),!i.content)throw e}else i=await E({providerId:l,baseURL:d,token:u,messages:r,model:f.activeModel,jsonMode:!1});let{displayContent:a,proposals:o}=Ct({response:i,wantsLogFood:v,hasImage:h}),p={chatId:c,role:`assistant`,content:a||i.content,nutritionProposals:o.length?$(o):void 0,nutritionProposal:o[0]?Q(o[0]):void 0,nutritionStatus:o.length?`pending`:void 0,providerId:l,createdAt:Date.now(),...j(i.usage,`chat`)};p.id=await m.chatMessages.add(p),n.value.push(p),await k(c,me(e,{toolInvocations:i.toolInvocations??[],userText:s})),await y(c),await M({providerId:l,model:i.model,kind:`chat`,usage:i.usage})}catch(e){throw i.value=e.message,e}finally{r.value=!1}}async function P(e){let{providerId:t,token:n,baseURL:r}=C(),i=await te({providerId:t,baseURL:r,token:n,audioBlob:e});return o.value=i.usage??null,await M({providerId:t,model:`whisper-1`,kind:`transcribe`,usage:i.usage}),i.text}async function F(e,t,r,i=0){let a=n.value.findIndex(t=>t.id===e);if(a===-1)return;let o=n.value[a],s=o.nutritionProposals?.length?$(o.nutritionProposals):o.nutritionProposal?[Q({...o.nutritionProposal,status:o.nutritionStatus??`pending`})]:[],c=s[i];if(!c||c.status===`confirmed`)return;let l=de(),u=ue(a),d=t||c.suggestedMealType||`lunch`;await l.addFromProposal(c,d,r,u),s[i]=Q({...c,status:`confirmed`}),await A(e,{nutritionProposals:s,nutritionStatus:s.every(e=>e.status===`confirmed`)?`confirmed`:`pending`})}async function pe(e){await A(e,{nutritionStatus:`dismissed`})}return{chats:e,activeChatId:t,messages:n,loading:r,error:i,loadChats:c,loadHistory:b,openChatSession:d,createChat:p,selectChat:_,deleteChat:v,send:N,transcribe:P,confirmNutrition:F,dismissNutrition:pe,updateMessage:A}}),Et={class:`bottom-nav`,"aria-label":`Основная навигация`},Dt={class:`bottom-nav__surface`},Ot={class:`bottom-nav__row`},kt={class:`bottom-nav__icon-wrap`},At={class:`bottom-nav__icon-wrap`},jt={class:`bottom-nav__icon-wrap`},Mt={class:`bottom-nav__icon-wrap`},Nt=M({__name:`BottomNav`,setup(t){let r=ee(),a=u(),o=Tt();function s(e){return e===`/`?r.path===`/`:e===`/settings`?r.path.startsWith(`/settings`):r.path===e}async function l(){await o.openChatSession(),r.path!==`/chat`&&await a.push(`/chat`)}return(t,r)=>{let a=d(`AppIcon`),o=d(`router-link`);return e(),c(`nav`,Et,[_(`div`,Dt,[r[1]||=_(`svg`,{class:`bottom-nav__shape`,viewBox:`0 0 360 88`,preserveAspectRatio:`none`,"aria-hidden":`true`},[_(`path`,{d:`M0 22C0 9.85 9.85 0 22 0H128C136.5 0 143.5 5.5 147 13.5C158.5 37.5 201.5 37.5 213 13.5C216.5 5.5 223.5 0 232 0H338C350.15 0 360 9.85 360 22V88H0V22Z`,fill:`#fff`})],-1),_(`div`,Ot,[i(o,{to:`/`,class:f([`bottom-nav__item`,{"bottom-nav__item--active":s(`/`)}]),"aria-label":`Главная`},{default:n(()=>[_(`span`,kt,[i(a,{icon:`lucide:home`,size:20})])]),_:1},8,[`class`]),i(o,{to:`/stats`,class:f([`bottom-nav__item`,{"bottom-nav__item--active":s(`/stats`)}]),"aria-label":`Статистика`},{default:n(()=>[_(`span`,At,[i(a,{icon:`lucide:bar-chart-3`,size:20})])]),_:1},8,[`class`]),r[0]||=_(`span`,{class:`bottom-nav__spacer`,"aria-hidden":`true`},null,-1),i(o,{to:`/foods`,class:f([`bottom-nav__item`,{"bottom-nav__item--active":s(`/foods`)}]),"aria-label":`Блюда`},{default:n(()=>[_(`span`,jt,[i(a,{icon:`lucide:apple`,size:20})])]),_:1},8,[`class`]),i(o,{to:`/settings`,class:f([`bottom-nav__item`,{"bottom-nav__item--active":s(`/settings`)}]),"aria-label":`Настройки`},{default:n(()=>[_(`span`,Mt,[i(a,{icon:`lucide:settings`,size:20})])]),_:1},8,[`class`])])]),_(`button`,{type:`button`,class:f([`bottom-nav__fab`,{"bottom-nav__fab--active":s(`/chat`)}]),"aria-label":`Новый чат`,onClick:l},[i(a,{icon:`lucide:plus`,size:28})],2)])}}},[[`__scopeId`,`data-v-6b296cbb`]]),Pt={class:`layout`},Ft={key:0,class:`layout__header`},It={key:1,class:`layout__subheader`},Lt=M({__name:`AppLayout`,props:{title:{type:String,default:`SafeCalorie`},showNav:{type:Boolean,default:!0},showHeader:{type:Boolean,default:!0},ellipsisTitle:{type:Boolean,default:!1}},setup(t){return(n,i)=>(e(),c(`div`,Pt,[t.showHeader?(e(),c(`header`,Ft,[_(`h1`,{class:f([`layout__title`,{"layout__title--ellipsis":t.ellipsisTitle}])},l(t.title),3),r(n.$slots,`header`,{},void 0,!0)])):s(``,!0),n.$slots.subheader?(e(),c(`div`,It,[r(n.$slots,`subheader`,{},void 0,!0)])):s(``,!0),_(`main`,{class:f([`layout__main`,{"layout__main--with-nav":t.showNav}])},[r(n.$slots,`default`,{},void 0,!0)],2),t.showNav?(e(),o(Nt,{key:2})):s(``,!0)]))}},[[`__scopeId`,`data-v-b2566cf6`]]);export{Tt as n,L as r,Lt as t};